    ...
    for (const name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        try {
          ...
          if (m.limit) {
            m.reply(`*${botConfig.emojis.info} Usaste ${m.limit} límite${m.limit > 1 ? 's' : ''}. Límite restante: ${global.db.data.users[m.sender]?.limit || 0}*`);
          }
        }
        break;
      }
        ...
        for (const user of participants) {
          try {
            ...
            if (userPrefix && chat.antiArab && botTt.restrict && isBotAdminNn && action === 'add') {
              ...
              await m?.conn?.sendMessage(id, { text: `*[] @${user.split('@')[0]} En este grupo no se permiten números árabes o raros, por lo que será eliminado del grupo.*`, mentions: [user] }, { quoted: fkontak2 });
              return;
            }
...
export async function groupsUpdate(groupsUpdate) {
  ...
  for (const groupUpdate of groupsUpdate) {
    ...
    if (groupUpdate?.subject) text = (chats?.sSubject || 'El título ha sido cambiado a:\n@subject').replace('@subject', groupUpdate.subject);
